...
/*   Updated: 2020/10/24 20:07:23 by ericlazo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_floorcasting(t_game *jeu)
{
	...
	while (y < jeu->file->res.y)
	{
		...
		while (x < jeu->file->res.x)
		{
			...
			ft_draw_pix_to_imge(jeu->fpv, x + y * jeu->file->res.x, color);

			ft_draw_pix_to_imge(jeu->fpv, (jeu->file->res.y - y - 1) * jeu->file->res.x + x , color);
